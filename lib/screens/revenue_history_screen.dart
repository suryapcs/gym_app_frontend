...
class _RevenueHistoryScreenState extends State<RevenueHistoryScreen> {
  ...
  String _formatMonthYear(String val) {
    ...
    try {
      ...
      if (parts.length == 2) {
        ...
        final year = parts[0];
        final monthName = DateFormat('MMMM').format(DateTime(int.parse(year), monthIdx));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Revenue History', style: TextStyle(fontWeight: FontWeight.w600)),
      ...
      body: _isLoading
          ...
          : _history.isEmpty
              ? const Center(child: Text('No historical revenue data found', style: TextStyle(color: Colors.grey)))
              : RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final totalIncome = double.tryParse(item['total_income']?.toString() ?? '0') ?? 0;
                      final totalExpenses = double.tryParse(item['total_expenses']?.toString() ?? '0') ?? 0;
  ...
  Widget _buildMetricBlock({required String label, required double amount, required Color color, required IconData icon}) {
    return Column(
      children: [
        Row(
